---
name: capture-check
description: Hail-Mary 카메라 캡처 스크립트(Camera/capture.py)를 실행하고 FPS·백엔드 상태를 점검한다. 카메라 관련 변경 후, 또는 Jetson에서 새 파이프라인을 시험할 때 사용.
---

# capture-check

## 목적
`Hail_Mary/Camera/capture.py`가 정상 동작하는지, 목표 FPS(≥10~15fps, 개발계획서 3.5절 M1 기준)를 만족하는지 빠르게 확인한다.
...
2. 헤드리스로 짧게 열어 FPS 확인 (창 없이, 자동 종료)
   ```
   python Hail_Mary/Camera/capture.py --no-preview --duration 5
   ```
...
3. 실제 화면으로 확인이 필요하면 미리보기 실행 (수동 종료: `q` 또는 창 닫기 버튼)
   ```
   python Hail_Mary/Camera/capture.py
   ```

4. Jetson에서 새 카메라/파이프라인을 처음 시험할 때
   ```
   python Hail_Mary/Camera/capture.py --backend gstreamer --pipeline jetson_csi --no-preview --duration 5
   ```
